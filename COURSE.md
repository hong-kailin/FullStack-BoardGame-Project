...
| 三 | React 重构 | 13 课 | 用 React + TypeScript 重构游戏界面 |
...
- 点击格子选中/取消选中
- 在 App.tsx 中用 `useState` 管理 `selectedPositions`
- 选中的格子高亮显示
- **产出**：点击版图格子会高亮
...
- 接收 `pyramid` 作为 props
- 渲染三个等级的卡牌（显示颜色、分数、王冠、费用）
- **产出**：页面上显示金字塔卡牌

### 第 38 课：PlayerInfo 组件
...
### 第 39 课：串联游戏流程 — 拿取标记
- 在 App.tsx 中接入 `handleTakeTokens`
- 添加"拿取标记"按钮
- 选中标记后点击按钮执行操作
- 游戏状态更新后自动重新渲染
- **产出**：可以在页面上拿取标记
...
### 第 41 课：串联游戏流程 — 回合切换
- 接入 `handlePass`（跳过回合按钮）
- 显示当前回合玩家
- 禁用非当前回合玩家的操作
- **产出**：完整的双人轮流操作流程

### 第 42 课：胜利条件与游戏结束
- 检测胜利条件（已在 game.ts 中实现）
- 显示胜利信息横幅
- 游戏结束后禁用所有操作
- **产出**：游戏可以正常结束

### 第 43 课：CSS 美化 React 版
- 给所有组件添加样式
- 版图网格、卡牌卡片、玩家面板
- 响应式布局
- **产出**：React 版游戏界面美观可用
...
### 第 44 课：为什么需要 monorepo？
### 第 45 课：动手搭建 Monorepo 骨架
### 第 46 课：拆分代码到子包
...
### 第 47 课：Node.js 原生 HTTP 服务器
### 第 48 课：引入 Express
### 第 49 课：文件存储用户数据
### 第 50 课：引入 SQLite 数据库
### 第 51 课：Session 与 Cookie
### 第 52 课：前端接入用户系统
