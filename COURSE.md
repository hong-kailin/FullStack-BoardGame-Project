...
| 四 | 用户系统（后端入门） | 7 课 | 注册、登录、Session、数据库 |
...
### 第 59 课：为什么需要 monorepo？（用刚遇到的共享痛点切入）
### 第 60 课：动手搭建 Monorepo 骨架（npm workspaces）
### 第 61 课：拆分代码到 core / web / server 子包
