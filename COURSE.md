...
| 子系统 | 当前状态 | 问题 |
|--------|----------|------|
| 标记回收/版图补充 | `bag` 字段为空数组，`takeTokens` 只移除不补充 | 版图标记越拿越少，后期全是空格 |
...
- 修改 `handleTakeTokens`：拿走的标记放回 `bag`
- 修改 `handleBuyCard`：花费的标记放回 `bag`
- 修改 `handleTakeGold`：黄金放回 `bag`
- 前端：版图空格自动出现新标记
- **产出**：版图标记用之不竭，游戏可以持续进行
