...
## 4. 补充：空的 cell 为什么比其他 cell 靠上？

打开页面后，你可能会发现空的格子（没有宝石的格子）比有宝石的格子位置更靠上，没有水平对齐。

原因是 CSS 中 `display: inline-block` 的元素默认是**基线对齐**（`vertical-align: baseline`）。

- 有文字的格子：基线是文字底部，格子被文字"撑"在正常位置
- 空的格子：没有文字，基线就是元素自身的底部，导致整个元素偏上

解决方法是给 `.cell` 加上 `vertical-align: top`，让所有格子统一顶部对齐：
...
`vertical-align` 控制 `inline-block` 元素在行内的垂直对齐方式。常用值：
...
对于网格类布局，用 `top` 最省心，所有格子顶部对齐，不会因为内容有无而错位。
